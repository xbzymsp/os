...
/*extern int tlb_visit_time;
extern int tlb_hit_time; */
...
if(vpn>32) break;
		...
		{
printf("i: %d vpn: %d ppn: %d\n\n",i,vpn,pageTable[i].physicalPage);
			entry = &pageTable[i];
			break;
		}
	}
...
printf("page fault!\n");
	...
	pageTable[vpn].last_use_time = stats->totalTicks;
	entry = &pageTable[vpn];
    } else {                              // tlb
//printf("tlb\n");
...
// lab4
		tlb[i].last_use_time = stats->totalTicks;
		entry = &tlb[i];			// FOUND!
		stats->tlb_hit_time++;
		//tlb_hit_time++;
		//printf("hit time: %d\n",tlb_hit_time);
